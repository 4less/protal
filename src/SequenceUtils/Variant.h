...
public:
    ...
    size_t QualitySum() const {
        if (IsReference()) return Observations() * 35;
        return std::accumulate(quals.begin(), quals.end(), 0, [](size_t acc, const uint16_t q) {
            return acc + q;
        });
    }
    size_t MeanQuality() const {
        return static_cast<size_t>(static_cast<double>(QualitySum())/quals.size());
    }
...
//            str += std::to_string(quality) + '\t';
    ...
    bool Match(Variant const& other) {
