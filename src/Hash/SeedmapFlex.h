...
//        inline bool operator > (LookupResult const& other) const = default;
    ...
    template<size_t ctrl_block_freq=8, size_t key_ubiquity_thresh=16>
    class SeedmapFlex {
    private:
        ...
        static constexpr size_t ctrl_block_frequency = ctrl_block_freq;
        ...
        size_t key_ubiquity_threshold = 16;
...
//        size_t ctrl_block_byte_size_shift = 2;
        ...
        SeedmapFlex(SeedmapFlex const& other) = delete;

        /**
         * Minikey functionality to keep returnvalues short
         */
        using MiniKey = uint16_t;
        using MiniSize = uint16_t;
        using MiniVal = uint16_t;
        struct ValueRange {
            const uint16_t start = 0;
            const uint16_t len = 0;

            ValueRange(uint16_t start, uint16_t len) :
                    start(start), len(len) {}
        };
        using ValueRangeList = std::vector<ValueRange>;
        static size_t GetBestRanges(ValueRangeList &ranges, MiniKey* keys, MiniSize size, MiniKey query, MiniVal max_error) {
            ranges.clear();
            MiniVal popcount_prev = 0;
            for (int i = 0; i < size; i++) {
                auto curkey = keys[i];
                auto curidx = i;
                MiniVal popcount = query != curkey;
                if (popcount > max_error) continue;
                while (++i < size && keys[i] == curkey);
                if (popcount >= popcount_prev) {
                    if (popcount > popcount_prev) ranges.clear();
                    ranges.emplace_back(ValueRange(curidx, i - curidx));
                    popcount_prev = popcount;
                }
            }
            return popcount_prev;
        }
...
#pragma omp critical(put)
{
        ...
        inline static size_t ComputeStartOffsetWorker(size_t count) {
            return (count+3)/4 + 1;
        }

        inline static size_t ComputeUpCountWorker(size_t count) {
            std::cout << "before: " << count << " -> ";
            count += ComputeStartOffsetWorker(count);
            ++count;
            std::cout << count << std::endl;
            return count;
        }

        inline size_t ComputeUpCount(size_t count) const {
            return count > key_ubiquity_threshold ?
                   ComputeUpCountWorker(count) :
                   count;
        }
        inline size_t ComputeValueOffset(size_t count) const {
            return count > key_ubiquity_threshold ?
                   ComputeStartOffsetWorker(count) :
                   0;
        }
...
//                std::cout << "new block" << std::endl;
                ...
                *control_ptr = global_position;
                ...
                for (auto key_pos = 0; key_pos < ctrl_block_frequency; key_pos++) {
                    size_t count = m_keymap[block_start_index + key_pos];
                    subkey[key_pos].key = key_pos;
                    subkey[key_pos].count = count;
                    size_t value_space_req = ComputeUpCount(count);

                    kmer_frequencies[subkey[key_pos].count]++;

                    total_count += value_space_req;
                }
...
// WHY? Figure out
                ...
                local_position = 0;
                for (auto key_pos = 0; key_pos < ctrl_block_frequency; key_pos++) {
                    auto global_key_pos = block_start_index + key_pos;
                    m_keymap[global_key_pos] = local_position;
                    m_keymap[global_key_pos] += ComputeValueOffset(subkey[key_pos].count);

                    size_t add = ComputeUpCount(subkey[key_pos].count);
                    local_position += add;
                    global_position += add;
                }
//                if (total_demand > 200) {
...
//            exit(9);
        ...
        void SortForKeys() {
            //TODO need to sort this time. ValueBlocks with Minikeys
            // Need to be sorted as lookup relies on them being grouped together
